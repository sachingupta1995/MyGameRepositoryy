...
public class RoomListActivity extends Activity implements ZoneRequestListener {
	...
	private void init(){
        try {
            theClient = WarpClient.getInstance();//Getting warp client already initialised
        } catch (Exception ex) {
	...
	public void onStart(){
		...
		theClient.getRoomInRange(1, 5);// trying to get room with at least one user
	}
	...
	//It is called when theClient.getRoomsinRange is called by the warpClient or theClient.getRoomWithProperties
	@Override
